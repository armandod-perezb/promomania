...
class _ExploreScreenState extends State<ExploreScreen>
  ...
  int _selectedTab = 1;        // Tab activa en navegación inferior (0=Inicio, 1=Explorar, 2=Favoritos, 3=Perfil)
  int _selectedSort = 0;       // Índice de opción de ordenamiento seleccionada
  bool _gridView = false;      // Control de vista: false=grid 2 columnas, true=lista 1 columna
  ...
  late final AnimationController _bannerCtrl;  // Controlador principal de animación
  late final Animation<double> _bannerFade;    // Animación de fade-in para el banner
  ...
  void initState() {
    ...
    _bannerCtrl = AnimationController(
      ...
      duration: const Duration(milliseconds: 600),   // Duración de la animación: 600ms
    );
                  ...
                  child: Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        ElevatedButton(
                          onPressed: () => promoService.init(),  // Reintentar inicialización
  ...
  Widget _buildHeader(PromoService promoService) {
    ...
    final currentUser = promoService.getUsuarios().isNotEmpty
        ...
        : null;
    final userCity = currentUser?.ciudad ?? 'Bogotá';  // Ciudad del usuario o fallback
              ...
              child: Text(
                ...
                currentUser != null
                    ? currentUser.nombre
                          ...
                          .map((name) => name[0])        // Tomar primera letra de cada palabra
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Container(
              ...
              child: const Row(
                children: [
                  SizedBox(width: 12),
                  Icon(Icons.search, color: Color(0xFFB0B5CC)),  // Icono de búsqueda
                  SizedBox(width: 8),
                  Text(
          ...
          Container(
            ...
            child: const Icon(Icons.tune, color: Colors.white),  // Icono de ajustes/filtros
          ),
        ],
      ),
    );
      ...
      children: [
            ...
            children: [
              Expanded(
                child: SingleChildScrollView(
                  scrollDirection: Axis.horizontal,  // Permitir scroll si el contenido es muy largo
                  child: Row(
                    children: [
        ...
        SizedBox(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (_, i) =>
                _buildFlashDealCard(flashDeals[i], promoService),  // Construir cada card
          ),
        ),
      ],
    );
    ...
    return Container(
      ...
      child: Center(
        child: Text(
          ...
          style: TextStyle(fontSize: height * 0.4),  // Tamaño proporcional a la altura
        ),
      ),
    );
  }
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    promo.titulo,
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,  // Cortar con puntos si es muy largo
                    style: const TextStyle(
                      fontSize: 13,
                      fontWeight: FontWeight.w800,
                      color: Color(0xFF1A1F2E),  // Gris oscuro
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (_, i) =>
                _buildNearbyCard(nearbyStores[i], promoService),  // Construir cada card
          ),
        ),
      ],
    );
  ...
  Widget _buildNearbyCard(
    ...
    final supermercado = storeData['supermercado'] as Supermercado;  // Datos de la tienda
    final promociones = storeData['promociones'] as List<Promocion>;  // Lista de promociones
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          GestureDetector(
            onTap: () => setState(() => _gridView = !_gridView),  // Cambiar vista
            child: Container(
              width: 34,
              height: 34,
              decoration: BoxDecoration(
                color: Colors.white,
                borderRadius: BorderRadius.circular(10),
                border: Border.all(color: const Color(0xFFE3E6EF)),  // Borde gris
              ),
              child: Icon(
                _gridView ? Icons.grid_view_rounded : Icons.view_agenda_rounded,  // Icono según vista
                size: 18,
  ...
  SliverPadding _buildPromosGrid(PromoService promoService) {
    ...
    final crossAxisCount = _gridView ? 1 : 2;  // 1 columna (lista) o 2 columnas (grid)
    ...
    return SliverPadding(
      ...
      sliver: SliverGrid(
        ...
        delegate: SliverChildBuilderDelegate(
          (context, index) =>
              _buildPromoGridCard(promociones[index], promoService),  // Construir cada card
